...
| 📺 室内自发光全息数据看板 (Holo Screen) | 🚀 全息数字孪生初始化控制舱 (Loading HUD) |
| :---: | :---: |
| ![室内自发光全息数据大屏](docs/images/16_holo_screen_telemetry.png) | ![全息初始化控制舱](docs/images/14_loading_hud.png) |
...
- **全域双视觉风格无损切换 (Dual-Theme Engine)**：
  - **🌙 赛博深空全息模式**：通透深晶蓝高反光地坪（`#20324d`）、深晶黑环体厂房、高折射微晶玻璃、荧光青蓝激光导航光带与暗黑磨砂玻璃 HUD，带来极致工业沉浸感；
  - **☀️ 白昼展厅沙盘模式**：通透素雅浅灰展台地坪（`#e2e8f0`）、物理骨架明暗高反差，最大化突显设备机械结构；
  - 支持顶部导航栏随时一键无损平滑切换，PBR 材质反射率与天光色温实时自适应联动。
- **仓内自发光全息态势监控看板 (Holo Telemetry Monitor)**：
  - 原位升级冷库正前方展示屏为 4.25m × 2.42m 高清自发光全息遥测大屏；
  - 动态渲染 7 台 AGV 激光寻引精度折线波形、恒温冷库（-18.2℃）气密环境、高位立体库容与 3 大月台泊位实时联动状态；
  - 采用双立柱工业级支架与微晶边框，配备独立定向发光与双模式材质自适应切换。
- **全息工业级 HUD 态势监控体系**：
